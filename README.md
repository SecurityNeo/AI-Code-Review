...
CodeGuard 不依赖单轮 LLM 直接输出结论，而是将审查任务编排为 11 个顺序执行阶段，覆盖从代码获取到报告输出的完整链路。密钥扫描、安全审计、测试建议、影响分析等阶段在本地规则匹配基础上，均支持可选 LLM 语义二次分析；分批评审和评审裁决阶段则以 LLM 为主进行问题发现与汇总评分。
...
| **密钥扫描** | 基于预编译正则规则扫描 diff 变更行，可选 LLM 语义二次验证 |
| **安全审计** | 基于 AST 提取的函数体进行关键词/正则匹配，可选 LLM 语义二次验证 |
| **测试建议** | AST 分析识别缺乏测试的变更函数，可选 LLM 增强生成测试用例 |
| **影响分析** | 对比基线与当前 AST 检测 Breaking Changes，可选 LLM 增强分析 |
