...
func initSecretScanRules() {
	...
	builtins := []SecretScanRule{
		...
		{Code: "SECRET-010", Name: "API Key 通用", Category: "generic", Severity: "high", Pattern: `(?i)(api[_-]?key|apikey)\s*[=:]\s*['"]?[A-Za-z0-9._-]{8,}['"]?`, Keywords: "apikey,api_key", Description: "API Key 硬编码", IsBuiltin: true, Enabled: true},
