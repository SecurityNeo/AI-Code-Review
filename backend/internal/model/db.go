...
func initMCPTools() {
	tools := []MCPTool{
		...
		{ToolName: "retry_task", DisplayName: "任务重试", Description: "重试 AI 评审任务，支持对已完成、失败、停止、超时、待处理状态的任务重新触发评审。review 任务可附加补充复核意见，AI 将参考意见进行针对性重审。", Category: "write", Icon: "fa-redo", Color: "#f59e0b", Tags: `["可写","需认证","需确认"]`, Params: `[{"n":"task_id","t":"integer","r":true},{"n":"user_review_comment","t":"string","r":false,"d":"补充复核意见，最多5000字符"},{"n":"selected_comment_ids","t":"array","r":false,"d":"历史复核意见ID列表"}]`, Phrases: `["重试任务 2084","重新评审一下","再跑一次","这个评审结果不对，重新审一下"]`, Dangerous: true, Requires: `["task_id"]`, SortOrder: 12},
