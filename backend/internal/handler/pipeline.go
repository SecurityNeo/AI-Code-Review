...
func (h *PipelineHandler) GetStageDetail(c *gin.Context) {
	...
	resp := gin.H{
		...
		// Token 预算相关字段（批次评审专用）
		"estimated_overhead": exec.EstimatedOverhead,
		"actual_overhead":    exec.ActualOverhead,
		"llm_input_budget":   exec.LLMInputBudget,
		"effective_budget":   exec.EffectiveBudget,
		"budget_warning":     exec.BudgetWarning,
	}
